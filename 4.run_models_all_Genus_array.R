...
#
#
#This script models bamboo genre in Peru
...
processing_genus <- unique(bambu_data$Genus)
...
bambu_genus_single <- bambu_data %>%
  dplyr::filter(Genus == genus)

bambu_genus_coordinates <- bambu_data %>%
  filter(Genus == genus) %>%
  st_coordinates()
#
cat(paste0("There are ", nrow(bambu_genus_single), " point records available for this genus..."), "\n")
# genus
cat("Calculating model data...", "\n")
model_data <- BIOMOD_FormatingData(resp.var = rep(1, nrow(bambu_genus_single)),
                                   expl.var = bambu_rasters,
                                   resp.xy = bambu_genus_coordinates,
                                   resp.name = paste0("Model_", str_replace(genus, " ", "_")),
                                   PA.strategy = "random",
                                   PA.nb.absences = 1000,
...
model_out <- BIOMOD_Modeling(model_data, models = model_algo,
                             ...
                             SaveObj = TRUE,
                             modeling.id = paste0(str_replace(genus, " ", "_"), "_model"))
...
write.csv(eval_values, paste0(csvs_folder, "/Model_assessment_", str_replace(genus, " ", "_"), ".csv"),
          row.names = FALSE)
...
write.csv(var_imp_values, paste0(csvs_folder, "/EMout_varimp_", str_replace(genus, " ", "_"), ".csv"),
          row.names = FALSE)
...
write.csv(eval_values_ensemble, paste0(csvs_folder, "/Ensemble_model_assessment_", str_replace(genus, " ", "_"), ".csv"),
          row.names = FALSE)
